...
class ContactForm(FlaskForm):
	first_name = StringField('first_name', validators=[DataRequired(), Length(min = 1, max = 100)])
...
class AccountForm(FlaskForm):
	name = StringField('first_name', validators=[DataRequired(), Length(min = 1, max = 100)])
	phone_number = IntegerField('phone_number', validators=[DataRequired()])
	email_address = StringField('email_address', validators=[DataRequired(), Email(message='INVALID_EMAIL_ADDRESS'), Length(min = 1, max = 256)])
